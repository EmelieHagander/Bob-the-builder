...
function VersionDialog({ projectId, id, revision, edit, areas, target, onClose, onSaved }: {
  ...
  return <Modal title={data ? `${data.title} · Version ${data.revision}` : 'Drawing version'} wide={Boolean(data?.parametricRecipe)} onClose={onClose}>
    ...
    {loading ? <Loading /> : error ? <Retry error={error} retry={() => setAttempt(value => value + 1)} /> : data && target && <>
      ...
      <div className="fact-list artifact-list">{data.drawings.items.map(item => {
        ...
        return <article className="card fact-card" aria-label={item.title} key={item.id}>
          ...
          <div className="foundation-actions">
            <button className="btn" onClick={() => setDialog({ kind: 'view', record: item })}>{item.parametricRecipe ? 'Open drawing' : 'View evidence'}</button>
