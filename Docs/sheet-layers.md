...
**Status:** implemented on `feat/sheet-layer-material-quantities` in PR #76; not merged, migrated or hosted/live-verified. The existing [material planning contract](material-planning.md) remains the owner of requirements, stock allocation, Shopping handoff and arithmetic. This document specifies its narrow sheet-layer method extension, not a parallel material model. Release evidence belongs on #76 until the separate release/documentation pass.
...
The prepared `scripts/check-live-sheet-layers.mjs` is an ordinary authenticated-client proof for the exact nonce-labelled disposable foundation Project. **It is not yet wired into the hosted runner or executed.** It must run after material planning and before the existing phase fixture changes effective targets. It preserves prior drawing and phase fixtures and uses no service key. Existing exact-project cleanup must include its task, stock, requirements and Shopping rows.
