...
**September 25 extension:** whole-project proposals now accept up to 100 Steps (previously 30), still preserving completed history and exact Task ownership. The same limit is used by tool schemas, parsing, the reviewed compiler and the authoritative SQL command; proposals remain bounded to 320 KB. One hundred Steps pass the SQL acceptance fixture; 101 are rejected. The compiler's output allowance is 24,000 tokens. This is a bounded whole-project plan, not an unlimited plan per Area.

Bob can now read task dependencies, tool/information needs and readiness through `read_project_work`, and change them through `manage_task_readiness`. The canonical work-plan command still rejects cycles and foreign-project links. A readiness review does not override blockers or imply completion. `save_project_build_day` maintains explicit existing Task IDs in `event_tasks`; the same Task can span days with one identity/status. Event detail reads that day's links, independently of RSVP, and shows errors/empty schedules honestly. Scheduling neither changes attendance nor claims volunteer availability.
...
## Focused plan edits and evidence continuity — September 25

`edit_project_plan` reads the complete current snapshot and patches only named
unfinished Steps/Completion Requirements. It supports changing fields, adding or
removing criteria/Steps, ordering Steps and explicitly moving existing Tasks. It
preserves unrelated identities and content without asking a compiler to reproduce
them. Completed history is immutable; removing obsolete work never certifies its
physical completion. Tasks from a removed Step remain visible as unorganised work
unless explicitly moved. The exact prepared result goes through the existing
proposal, revision, access and decision commands.

The compiler remains available for new/broad plans. Its current revision is bound
by the server, not regenerated by the model. Only explicit Task ownership changes
are staged, with one owner per Task. Saving the prepared proposal exposes the
normal decision path; an instruction to apply a specific edit can authorise that
exact decision in the same turn. A request to explore alternatives cannot.

Unchanged criteria retain their pinned evidence across proposal and approval,
including evidence linked while a proposal was pending. Changed criteria do not
inherit proof. Relinking a revised evidence record replaces its old revision pin
and serializes with plan approval. Source freshness checks still apply.

`plan-edits.test.ts` exercises removal/reordering, identity preservation, late
evidence, changed criteria, stale revisions and another caller's denial through
the full database schema. Controlled runtime tests cover continuation of a second
authorised subtask after plan failure. These establish the mechanism, not a
named-member real-model acceptance result. The release PR owns hosted evidence.
