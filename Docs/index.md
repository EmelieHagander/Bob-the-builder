...
| reusable material/part definitions, dynamic profiles and catalog tools | `Docs/material-catalog.md` — slice A implementation branch #94; not deployed, no assembly geometry yet |
| generic CAD engine adapter / construction geometry seam | `Docs/cad-adapter.md` — build123d/Open Cascade adapter foundation in #96; not deployed or yet persisted as an Artifact assembly |
...
- `Docs/material-catalog.md` — **slice A implementation owner, not deployed**: material/part definition kinds sharing version mechanics, dictionary/profile data, exact unit normalization, conservative ensure/revise, caller authority and catalog tool integration. Refines the usecase's proposed table names; no generic drawing/BOM/Shopping completion claim.
- `Docs/cad-adapter.md` — **Slice B CAD seam implementation owner, not deployed**: strict mm construction contract, pinned build123d/Open Cascade worker, STEP/SVG outputs and adapter failure boundary. It is not yet an Artifact persistence model, hosted CAD service, BOM or Shopping implementation.
...
- `Docs/material-catalog.md` + `supabase/migrations/20260921095817_material_catalog.sql` — first catalog implementation schema/authority; source only until the release gates are recorded. `catalog_items.kind` keeps material and part definitions distinct while sharing immutable revision mechanics.
