...
| generic CAD engine adapter / construction geometry seam | `Docs/cad-adapter.md` — merged build123d/Open Cascade adapter foundation; real engine seam verified, not yet hosted/persisted as an Artifact assembly |
...
- `Docs/living-project-plan.md` — **specified product owner for Bob's future living plan model**: dynamic Project Plan → Step → Completion Requirement → Evidence/Responsibility, shared-current-state briefing, supersession/refinement and explicit AI replan proposals. It does not claim the current task/readiness schema already implements this model.
...
- `Docs/cad-adapter.md` — **Slice B CAD seam implementation owner, merged but not hosted as a Bob CAD service**: strict mm construction contract, pinned build123d/Open Cascade worker, STEP/SVG outputs and adapter failure boundary. It is not yet an Artifact persistence model, hosted CAD service, BOM or Shopping implementation.
