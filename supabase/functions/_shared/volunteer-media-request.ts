...
export function createVolunteerMediaHandler(deps: {
  authorize: (session: string, taskId: string, mediaId: string) => Promise<VolunteerImage | null>
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(key => !['session', 'taskId', 'mediaId'].includes(key))
      ...
      const image = await deps.authorize(body.session, body.taskId, body.mediaId)
      ...
      const current = await deps.authorize(body.session, body.taskId, body.mediaId)
