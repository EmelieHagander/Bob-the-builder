...
export function createBobToolSession(opts: {
  ...
  const definitions: ToolDefinition[] = [
    ...
    ...(opts.cadAssistant ? [
      ...opts.cadAssistant.tools.map(spec => ({spec,version:2,offerWhenReady:()=>opts.drawingRequested?.()??false,gate:():ToolGate=>opts.cadAssistant!.remaining>0?'available':'budget_exhausted',execute:(v:unknown)=>opts.cadAssistant!.consult(v)})),
