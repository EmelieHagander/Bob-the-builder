...
export const RECORD_DETAIL_TOOL={type:'function' as const,function:{name:'read_project_record_section',description:'Read a section of a large plan or CAD record. Start with path=[]; too_large returns keys or array_length so you can choose a narrower JSON path. For a plan use the revision as record_id; for CAD use the Artifact UUID. Exact revision is required for CAD. This is project-bound read-only data.',parameters:{type:'object',additionalProperties:false,properties:{dataset:{type:'string',enum:['plan','cad']},record_id:{type:'string'},revision:{type:['integer','null']},path:{type:'array',maxItems:8,items:{type:'string'}}},required:['dataset','record_id','revision','path']}}}
export function createRecordDetailReader(read:(dataset:'plan'|'cad',id:string,revision:number|null)=>Promise<unknown>,hasAccess:()=>Promise<boolean>){let used=0
 ...
 if(Object.keys(v).length!==4||!['plan','cad'].includes(v.dataset)||typeof v.record_id!=='string'||v.record_id.length>200||!Array.isArray(v.path)||v.path.length>8||v.path.some((s:unknown)=>typeof s!=='string'||s.length>100||['__proto__','prototype','constructor'].includes(s))||!(v.revision===null||Number.isSafeInteger(v.revision)&&v.revision>0))return {status:'invalid'}
 if(v.dataset==='cad'&&v.revision===null||v.dataset==='plan'&&!/^[1-9][0-9]{0,8}$/.test(v.record_id))return {status:'invalid'}
