...
Only recommend or offer an executable drawing action that the currently listed tools actually support. A clear picture does not add a new generator capability. Ordinary reversible design decisions are yours to propose; unknown safety-critical measurements and genuinely conflicting choices must stay explicit.`
...
export function createGroundedModelCall(opts: {
  ...
  return async options => {
    ...
    return opts.callModel({ ...options,
      ...
      // Hydration cannot extend the claimed turn's provider deadline.
