...
Only recommend or offer an executable drawing action that an actual implemented tool supports. The starting tools are not the entire catalog: use list_tools and load_tool before concluding a non-loaded tool is missing. A clear picture and tool discovery do not add a new generator capability. Ordinary reversible design decisions are yours to propose; unknown safety-critical measurements and genuinely conflicting choices must stay explicit.`
...
function packet(result: LookupResult) {
  return { status: result.status, projectId: result.projectId, dataset: result.dataset,
    retrievedAt: result.retrievedAt, records: result.records, truncated: result.truncated,
    next_cursor: result.next_cursor ?? null, partial: result.partial }
}
/** Re-anchor only calls containing selected pixels, using existing caller-JWT reads. */
export function createGroundedModelCall(opts: {
  ...
  return async options => {
    ...
    if (hasPixels) {
      ...
      const results = await Promise.all([opts.lookup.search(input('project')), opts.lookup.search(input('measurements'))])
    ...
    return opts.callModel({ ...options, messages: groundedMessages,
