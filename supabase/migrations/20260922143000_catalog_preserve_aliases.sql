-- Preserve existing catalog aliases on ordinary revisions when Bob is not
-- explicitly replacing them. Empty aliases on revise mean "preserve"; the
-- current catalog tool does not expose alias deletion.
begin;
...
begin
 ...
 if action='ensure' then
   ...
   -- Revise is a specification edit, not an implicit alias deletion. The
   -- current tool has no explicit "clear aliases" operation, so an empty
   -- aliases array preserves the current aliases. A non-empty array remains an
   -- explicit replacement.
   if cardinality(aliases_input)=0 and cardinality(current_rev.aliases)>0 then
     aliases_input:=current_rev.aliases;
   end if;
...
set how_to = how_to ||
  ' On revise, preserve existing aliases unless the user explicitly supplied replacement aliases. With the current tool contract an empty aliases array means preserve, not delete; alias deletion is not exposed. Read the current definition before revising so unrelated metadata is retained.'
