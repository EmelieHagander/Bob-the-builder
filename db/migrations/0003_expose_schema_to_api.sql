...
--
-- ORDERING IS LOAD-BEARING: 0001 (create schema) MUST run before this file.
-- PostgREST fails its entire schema-cache load — taking every app on the
-- shared API down with PGRST002/503 — if any schema in `pgrst.db_schemas`
-- does not exist. This happened on 2026-07-11; the guard below now refuses
-- to expose a schema that hasn't been created yet.
--
...
begin
  ...
  if not exists (select 1 from pg_namespace where nspname = 'bob') then
    raise exception 'bob 0003: schema "bob" does not exist — run 0001_create_bob_schema.sql first. '
      'Exposing a nonexistent schema makes PostgREST''s schema-cache load fail, '
      'which takes the WHOLE shared Data API down (PGRST002/503 for every app).';
  end if;
