...
path = 'src/pages/MaterialPlan.tsx'
replace(path, "  const [params] = useSearchParams()", "  const [params, setParams] = useSearchParams()")
replace(path, "    value.targetChanged && 'project target',", "    value.targetChanged && 'selected target',")
replace(path,
...
replace(path,
    ...
    "          <Field label=\"Scope\"><input style={inputStyle} value={area ? scopeName : 'Project as a whole'} disabled /></Field>\n          <Field label=\"Task\"><select style={inputStyle} value={task} onChange={event => setTask(event.target.value)}><option value=\"\">No task yet</option>{taskOptions.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>", count=2)
replace(path,
...
old_dialog = '''function RequirementVersionDialog({ projectId, id, revision, target, edit, areas, tasks, artifacts, stock, components, onClose, onSaved }: {
...
new_dialog = '''function RequirementVersionDialog({ projectId, id, revision, scopeArea, edit, areas, tasks, artifacts, stock, components, onClose, onSaved }: {
...
replace(path, old_dialog, new_dialog)
replace(path, "  const [detail, setDetail] = useState<{ id: string; revision: number; edit?: boolean } | null>(null)", "  const [detail, setDetail] = useState<{ id: string; revision: number; areaId: string; edit?: boolean } | null>(null)")
replace(path, "      db.getSelectedTarget(projectId), db.getAreas(), db.getTasks(), db.getProjectArtifacts(projectId, '', false, 0),", "      db.getSelectedTarget(projectId, areaFilter), db.getAreas(), db.getTasks(), db.getProjectArtifacts(projectId, '', false, 0),")
replace(path,
...
replace(path, "{selected && !generatedArtifacts.length && <p className=\"foundation-hint\">Need a supported wall calculation? <Link to=\"/artifacts\">Generate a wall elevation in Plans & drawings</Link> first.</p>}", "{selected && !generatedArtifacts.length && <p className=\"foundation-hint\">Need a supported wall calculation? <Link to={`/artifacts${scopeQuery}`}>Generate a wall elevation in Plans & drawings</Link> first.</p>}")
replace(path,
    "<Field label=\"Filter by area\"><select style={inputStyle} value={areaFilter} onChange={event => { setAreaFilter(event.target.value); setOffset(0) }}><option value=\"\">All areas</option>{data.areas.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>",
    "<Field label=\"Filter by area\"><select style={inputStyle} value={areaFilter} onChange={event => { const next = event.target.value; setAreaFilter(next); setOffset(0); setParams(next ? { area: next } : {}, { replace: true }) }}><option value=\"\">All areas</option>{data.areas.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>")
replace(path, "setDetail({ id: item.id, revision: item.revision })", "setDetail({ id: item.id, revision: item.revision, areaId: item.areaId ?? '' })")
replace(path, "setDetail({ id: item.id, revision: item.revision, edit: true })", "setDetail({ id: item.id, revision: item.revision, areaId: item.areaId ?? '', edit: true })")
replace(path, "{data && editor === 'new' && selected && <RequirementEditor projectId={projectId} target={data.target} areas={data.areas} tasks={data.tasks} artifacts={data.artifacts}", "{data && editor === 'new' && selected && <RequirementEditor projectId={projectId} target={data.target} scopeArea={areaFilter} areas={data.areas} tasks={data.tasks} artifacts={scopeArtifacts}")
replace(path, "{data && calculator && selected && generatedArtifacts.length > 0 && <DeterministicRequirementEditor projectId={projectId} target={data.target} areas={data.areas}", "{data && calculator && selected && generatedArtifacts.length > 0 && <DeterministicRequirementEditor projectId={projectId} target={data.target} scopeArea={areaFilter} areas={data.areas}")
replace(path, "{data && detail && <RequirementVersionDialog projectId={projectId} id={detail.id} revision={detail.revision} edit={detail.edit} target={data.target}", "{data && detail && <RequirementVersionDialog projectId={projectId} id={detail.id} revision={detail.revision} scopeArea={detail.areaId} edit={detail.edit}")
replace(path, "setDetail({ id: history.id, revision })", "setDetail({ id: history.id, revision, areaId: history.areaId ?? '' })")

path = 'scripts/solutions-browser.mjs'
replace(path,
...
replace(path, "      if (table === 'current_target') rows = decisions.length ? [decisions.at(-1)] : []", "      if (table === 'current_target') {\n        const latest = new Map()\n        for (const item of decisions) latest.set(item.scope_key ?? (item.area_id ? `area:${item.area_id}` : 'project'), item)\n        rows = [...latest.values()]\n      }")

path = 'scripts/artifacts-browser.mjs'
replace(path, "    reason: 'Use the extension for the drawing fixture', actor_label: 'Fixture member', recorded_at: selected.recorded_at,\n  })", "    reason: 'Use the extension for the drawing fixture', actor_label: 'Fixture member', recorded_at: selected.recorded_at,\n    area_id: null, scope_key: 'project',\n  })")
replace(path, "    reason: 'Change target after drawing', actor_label: 'Fixture member', recorded_at: other.recorded_at,\n  })", "    reason: 'Change target after drawing', actor_label: 'Fixture member', recorded_at: other.recorded_at,\n    area_id: null, scope_key: 'project',\n  })")

path = 'scripts/material-planning-browser.mjs'
replace(path, "  const currentTarget = projectId => {\n    const decision = [...solutions.decisions].reverse().find(item => item.project_id === projectId)\n    if (!decision?.solution_id) return null", "  const currentTarget = (projectId, areaId = null) => {\n    const decisions = [...solutions.decisions].reverse().filter(item => item.project_id === projectId)\n    const scoped = decisions.find(item => (item.area_id ?? null) === areaId)\n    const decision = scoped ?? (areaId ? decisions.find(item => item.area_id == null) : null)\n    if (!decision?.solution_id) return null")
replace(path, "    const target = currentTarget(row.project_id)", "    const target = currentTarget(row.project_id, row.area_id ?? null)")
replace(path, "        const target = currentTarget(p_project)", "        const target = currentTarget(p_project, data.area_id ?? null)", count=2)
replace(path, "  await stockCard.getByText(/1 pcs/).waitFor()\n\n  await page.getByRole('button', { name: 'Add requirement', exact: true }).click()", "  await stockCard.getByText(/1 pcs/).waitFor()\n\n  await page.getByLabel('Filter by area', { exact: true }).selectOption('areaA')\n  await page.getByRole('heading', { name: 'Entry target', exact: true }).waitFor()\n  await page.getByText(/Inherited from Project/).waitFor()\n  await page.getByRole('button', { name: 'Add requirement', exact: true }).click()")
replace(path, "  await modal.getByLabel('Area', { exact: true }).selectOption('areaA')\n  await modal.getByLabel('Task', { exact: true }).selectOption('taskA')", "  assert.equal(await modal.getByLabel('Scope', { exact: true }).inputValue(), 'Entry')\n  await modal.getByLabel('Task', { exact: true }).selectOption('taskA')", count=2)
replace(path, "  await page.getByRole('button', { name: 'Calculate from drawing', exact: true }).click()", "  await page.getByLabel('Filter by area', { exact: true }).selectOption('areaA')\n  await page.getByRole('button', { name: 'Calculate from drawing', exact: true }).click()")
replace(path, "  await page.getByText('Choose a project target before recording material requirements.', { exact: true }).waitFor()", "  await page.getByText('Choose a target for the Project before recording material requirements.', { exact: true }).waitFor()")
replace(path, "  console.log(`Material plan manual + deterministic drawing quantity, stock/reuse/arithmetic/Shopping handoff/reload/project isolation passed at ${width}px; HTTP fixtures, no AI.`)", "  console.log(`Material plan Project/Area target scope + manual/deterministic quantity, stock/reuse/arithmetic/Shopping handoff/reload/project isolation passed at ${width}px; HTTP fixtures, no AI.`)")
