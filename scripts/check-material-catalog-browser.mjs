...
try {
  ...
  for(const viewport of [{width:320,height:568},{width:390,height:844},{width:1280,height:900}]) {
    ...
    assert.equal(histories.get('A').messages.at(-1).evidence.writes[0].revision,1)
    ...
    assert.equal(await drawer.getByText('Saved to project',{exact:true}).count(),0)
    ...
    assert.equal(histories.get('A').messages.at(-1).evidence.writes[0].revision,2)
    ...
    assert.equal(requests.filter(r=>r.clientTurnId===turnId).length,1,'Lost reply recovered without resending')
    assert.equal(await drawer.getByText('Saved to project',{exact:true}).count(),0)
    assert(await drawer.evaluate(n=>n.scrollWidth<=n.clientWidth+1),'Catalog receipts must not overflow horizontally')
    await drawer.getByText('Same definition revised.',{exact:true}).scrollIntoViewIfNeeded()
